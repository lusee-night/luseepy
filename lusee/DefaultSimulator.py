...
class DefaultSimulator(SimulatorBase):
    ...
    def simulate (self,times=None):
        ...
        # Optional plotting of sky and beam at first timestep
        if self.extra_opts.get("plot_sky_and_beam"):
            ...
            # Compute rotated sky for first timestep if needed
            if do_rot:
                alpha, beta, gamma = self._compute_zyz_angles(lzl[:1], bzl[:1], lyl[:1], byl[:1], 1)
                sky_t0 = self._rotate_many_times_jax(
                    sky_base,
                    jnp.asarray(alpha),
                    jnp.asarray(beta),
                    jnp.asarray(gamma),

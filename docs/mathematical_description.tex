...
The \texttt{JaxSimulator} path implements the equivalent contraction on the full $(\ell, m\in[-\ell,\ell])$ grid, reconstructing the negative-$m$ modes via
\[
...
\left\langle a, s \right\rangle
=
\frac{1}{4\pi}\,\RePart\!\left[\sum_{\ell=0}^{L}\sum_{m=-\ell}^{\ell} a_{\ell m}\,\overline{s_{\ell m}}\right].
\]
...
\item The rotation is applied to the harmonic coefficients:
\[
S_t(\nu) = \mathcal{R}(R_t)\,S(\nu).
\]
\end{enumerate}

In \texttt{DefaultSimulator} this is implemented by \texttt{healpy.Rotator} acting directly on the packed alm array. In \texttt{JaxSimulator}, $R_t$ is instead converted to ZYZ Euler angles $(\alpha_t,\beta_t,\gamma_t)$ matched to \texttt{healpy}'s XYZ convention, and the rotation is applied to the full $(\ell,m)$ sky array via Wigner-$d$ recursions (\texttt{s2fft}'s \texttt{compute\_full}):
\[
s_{\ell m}^{(t)}
=
\sum_{m'=-\ell}^{\ell}

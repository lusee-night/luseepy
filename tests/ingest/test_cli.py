...
def test_session_failure_prints_actual_result_paths_and_issues(
    tmp_path,
    monkeypatch,
    capsys,
):
    h5_path = tmp_path / "h5" / "resolved-name.h5"
    manifest_path = tmp_path / "manifests" / "resolved-name.json"
    h5_path.parent.mkdir()
    manifest_path.parent.mkdir()
    h5_path.write_bytes(b"partial hdf5")
    manifest_path.write_text("{}", encoding="ascii")
    failure = pipeline.SessionResult(
        session_ordinal=0,
        session_name="resolved-name",
        source_path=str(tmp_path / "input"),
        source_kind="session",
        status="failed",
        issue_counts={"writer.failed": 1},
        status_issue_codes=["writer.failed"],
        h5_path=str(h5_path),
        manifest_path=str(manifest_path),
    )
    error = RuntimeError("writer failed")
    error.ingest_result = failure
    monkeypatch.setattr(
        cli.pipeline,
        "process_session",
    ...
    assert f"hdf5: {h5_path}" in captured.out
    assert f"manifest: {manifest_path}" in captured.out
    assert "issues: writer.failed=1" in captured.out
    assert captured.err == "error: RuntimeError: writer failed\n"


def test_flash_failure_prints_existing_results_and_issues(
    tmp_path,
    monkeypatch,
    capsys,
):
    manifest_path = tmp_path / "sessions" / "flash.json"
    manifest_path.parent.mkdir()
    manifest_path.write_text("{}", encoding="ascii")
    failure = pipeline.FlashResult(
        flash_result_id=f"flash-{'a' * 16}",
        input_identity_sha256=None,
        source_path=str(tmp_path / "flash"),
        source_fingerprint={},
        status="failed",
        issue_counts={"pipeline.flash_failed": 1},
        status_issue_codes=["pipeline.flash_failed"],
        manifest_paths=[
            str(tmp_path / "missing" / "flash.json"),
            str(manifest_path),
        ],
    )
    error = RuntimeError("flash failed")
    error.ingest_result = failure
    monkeypatch.setattr(
        cli.pipeline,
        "process_flash",
    ...
    assert "status: failed" in captured.out
    assert f"manifest: {manifest_path}" in captured.out
    assert "missing/flash.json" not in captured.out
    assert "issues: pipeline.flash_failed=1" in captured.out
    assert captured.err == "error: RuntimeError: flash failed\n"

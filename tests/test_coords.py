...
def observation():
    return Observation(2501, deltaT_sec=12 * 24 * 3600)


def test_jupiter_track(observation):
    alt, az = observation.get_track_solar("jupiter")
...
def test_crab_track(observation):
    alt, az = observation.get_track_ra_dec(ra="05h34m31.94s", dec="+22d00m52.2s")
...
def test_lpf_track(observation):
    S = Satellite()
    OS = ObservedSatellite(observation, S)
